// В программе реализована обработка данных для фитнес браслета
// При беге, ходьбе и плавании
package main
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	meenSpeed := float64(r.Action) * LenStep / MInKm / r.Duration.Hours()
	result := ((CaloriesMeanSpeedMultiplier*meenSpeed + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
	return result
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	meenSpeedMS2 := (float64(w.Action) * LenStep / MInKm / w.Duration.Hours() * KmHInMsec) * (float64(w.Action) * LenStep / MInKm / w.Duration.Hours() * KmHInMsec)
	result := ((CaloriesWeightMultiplier*w.Weight + (meenSpeedMS2/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return result
}
...
const (
	SwimmingLenStep                  = 1.38 // длина одного гребка (так и не пригодилась в программе)
